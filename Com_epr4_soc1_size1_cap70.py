# -*- coding: utf-8 -*-
"""Copy of Optimization model.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1C9uAfwtzqmoivAISVDcyiKLA9kEYQnE4
"""
...
for n in range(0, n_years): #or range(1, n_years+1)
    ...
    #if (sum(daily_degrad) <= model.degradation_limit):
    ...
    YB.append(sum(DBU) / ((1 + model.discount) ** n))
...
for i in range(0, len(YB)):
    #Y_OM_cost.append((rem_energy - Y_degrad[i]) / 4 * om_cost)
...
df1 = pd.DataFrame(index=np.arange(0, len(YB)), columns=('Year', 'Savings', 'Degradation', 'Throughput', 'Deg Cost', 'SOH', 'OM Cost', 'NPV (excl. CAPEX)', 'Misc'))
...
for i in range(len(YB)):
    ...
    df1.iloc[i, 6] = (om_cost * (df1.iloc[i, 5] * rated_energy / 4)) / ((1 + df) ** i)
    if i == 0:
        df1.iloc[i, 7] = df1.iloc[i, 1] - df1.iloc[i, 6] 
    else:
